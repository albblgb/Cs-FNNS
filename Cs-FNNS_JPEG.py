...
from utils.distortion_layers import jpeg_compression_layer, img_jpeg_compression
import config as c


secret_dataset = c.secret_dataset_dir.split('/')[-2]
cover_dataset = c.cover_dataset_dir.split('/')[-2]
logger_name = 'Cs-FNNS_JPEG'
image_save_dirs = os.path.join('results', 'Cs-FNNS_JPEG', secret_dataset)
...
stego_psnr_list = []; stego_ssim_list = []; stego_lpips_list = [0]; stego_apd_list = []
secret_rev_psnr_list = []; secret_rev_ssim_list = []; secret_rev_lpips_list = [0]; secret_rev_apd_list = []
...
for i in range(len(secret_image_path_list)):  
    ...
    for iteration_index in range(c.iters):
        ...
        if c.add_jpeg_layer  == True and (iteration_index + 1) % 2 == 0:
            adv_pert = jpeg_compression_layer(adv_pert, cover)
    ...
    adv_pert = img_jpeg_compression(adv_image) - cover
    # testing 
